...
def check_worksheets
  # check_worksheet("1WZHBdOqfJc6pbpizQHtInj4bSW8jduKjgPzjILml0UY")
  spreadsheet_keys.reverse.take(20).each_with_index { |key, i| puts i; check_worksheet(key) }
end
...
def check_worksheet(spreadsheet_key)
  ...
  full_people_row = GoogleSheet.range(spreadsheet_key, "#{start_row}:#{start_row}")
  raise "Spreadsheet with key: #{spreadsheet_key} is in an invalid state" unless full_people_row_valid? full_people_row[:values].first
end
